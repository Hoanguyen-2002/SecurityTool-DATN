...
interface ModalProps {
  ...
  confirmButtonPosition?: 'left' | 'right'; // New prop for button order
}
...
const Modal: React.FC<ModalProps> = ({
  ...
  confirmButtonPosition = 'right', // Default to 'right'
}) => {
  ...
  const confirmButton = showConfirmButton && onConfirm && (
    <button
      onClick={onConfirm}
      ...
      <div className={`bg-white p-5 rounded-lg shadow-xl w-full ${maxWidthClass} mx-auto`}>
        ...
        {shouldRenderFooter && (
          <div className="mt-6 flex justify-end space-x-2">
            {confirmButtonPosition === 'left' && confirmButton}
            {cancelButton}
            {confirmButtonPosition === 'right' && confirmButton}
          </div>
        )}
      </div>
    </div>
