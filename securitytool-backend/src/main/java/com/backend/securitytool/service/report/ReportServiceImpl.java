...
public class ReportServiceImpl implements ReportService {
    ...
    public ReportResponseDTO getReport(Integer resultId) {
        logger.debug("Fetching report for resultId: {}", resultId);
        Page<SecurityIssue> issuesPage = securityIssueRepository.findByResultAppId(resultId, PageRequest.of(0, 100));
        List<SecurityIssue> issues = issuesPage.getContent();
        ...
        logger.info("Retrieved {} issues for report", issues.size());
        return report;
    }

    public String exportCsv(Integer resultId) {
        logger.debug("Exporting CSV for resultId: {}", resultId);
        List<SecurityIssue> issues = securityIssueRepository.findByResultAppId(resultId);
